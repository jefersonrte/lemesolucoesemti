...
function tenderFilterUrl(array $filters): string
{
    return '/gov/licitacoes.php?' . http_build_query($filters, '', '&', PHP_QUERY_RFC3986) . '#licitacoes';
}
...
$situation = (string) ($_GET['situacao'] ?? 'abertas');
...
$meta = [
    ...
    'pagina' => 1,
    'totalPaginas' => 1,
    'totais' => ['todos' => 0, 'abertas' => 0, 'emAndamento' => 0, 'ti' => 0, 'obras' => 0, 'saude' => 0, 'seguranca' => 0],
...
foreach ($cities as $municipality) {
    if (($municipality['slug'] ?? '') === $city) {
        $selectedCityName = (string) ($municipality['nome'] ?? '');
        break;
    }
}
$resultsTitle = match ($situation) {
    'encerradas' => $selectedCityName !== '' ? 'Histórico de ' . $selectedCityName : 'Licitações encerradas',
    'todas' => $selectedCityName !== '' ? 'Todas as licitações de ' . $selectedCityName : 'Todas as licitações',
    default => $selectedCityName !== '' ? 'Compras abertas de ' . $selectedCityName : 'Compras abertas',
};
...
<body>
  <main class="wrap">
    ...
    <header class="hero">
      <div><span class="eyebrow">GRANDE FLORIANÓPOLIS / MONITOR DE OPORTUNIDADES</span><h1>Licitações com foco em <span>TI</span></h1><p>Consulta pública sem senha. Os dados ficam armazenados neste site; compras abertas são identificadas por prazo de propostas futuro e vínculo com o processo oficial correto.</p></div>
    ...
    <section class="metrics" aria-label="Resumo">
      ...
      <div class="metric"><strong><?= number_format((int) ($totals['abertas'] ?? ($totals['emAndamento'] ?? 0)), 0, ',', '.') ?></strong><span>abertas para propostas</span></div>
    ...
    <section class="section" id="portais">
      <div class="section-head"><div><span class="tag">DADOS BAIXADOS NO SITE</span><h2>Grande Florianópolis e região</h2><p>Clique no nome da cidade ou em “Ver compras abertas” para listar apenas oportunidades com prazo vigente. O histórico completo permanece pesquisável no seu site.</p></div></div>
      <div class="portals">
        <?php foreach ($cities as $municipality):
          $municipalitySlug = (string) ($municipality['slug'] ?? '');
          $openCityUrl = tenderFilterUrl(['cidade' => $municipalitySlug, 'situacao' => 'abertas']);
          $historyCityUrl = tenderFilterUrl(['cidade' => $municipalitySlug, 'situacao' => 'todas']);
          $openCityCount = (int) ($municipality['abertas'] ?? ($municipality['emAndamento'] ?? 0));
          $openCityTi = (int) ($municipality['tiAbertas'] ?? ($municipality['tiEmAndamento'] ?? 0));
        ?>
          <article class="portal<?= $city === $municipalitySlug ? ' selected' : '' ?>">
            <div class="portal-top"><span class="tag"><?= tenderH($municipality['fonte'] ?? 'OFICIAL') ?></span><span class="count"><?= number_format($openCityCount, 0, ',', '.') ?></span></div>
            <h3><a href="<?= tenderH($openCityUrl) ?>"><?= tenderH($municipality['nome'] ?? '') ?></a></h3>
            <p><?= number_format($openCityTi, 0, ',', '.') ?> de TI abertas · <?= number_format((int) ($municipality['total'] ?? 0), 0, ',', '.') ?> registros armazenados.</p>
            <div class="portal-actions"><a class="primary" href="<?= tenderH($openCityUrl) ?>">Ver compras abertas →</a><a href="<?= tenderH($historyCityUrl) ?>">Todo o histórico</a><a href="<?= tenderH($municipality['portal'] ?? '#') ?>" target="_blank" rel="noopener noreferrer">Portal oficial ↗</a></div>
          </article>
        <?php endforeach; ?>
      </div>
    </section>
    ...
    <section class="section" id="licitacoes">
      <div class="section-head"><div><span class="tag">BUSCA PÚBLICA / SEM LOGIN</span><h2><?= tenderH($resultsTitle) ?></h2><p>Licitações de TI aparecem primeiro. “Aberta” significa que o prazo final para envio de propostas ainda não venceu.</p></div></div>
      <form class="tools" method="get" action="/gov/licitacoes.php#licitacoes" role="search">
        ...
        <select name="situacao" aria-label="Filtrar por situação">
          <option value="abertas"<?= $situation === 'abertas' ? ' selected' : '' ?>>Abertas para propostas</option>
          <option value="encerradas"<?= $situation === 'encerradas' ? ' selected' : '' ?>>Prazo encerrado</option>
          <option value="todas"<?= $situation === 'todas' ? ' selected' : '' ?>>Todas as situações</option>
        </select>
        <button type="submit">PESQUISAR</button>
      </form>
      <div class="quick"><a class="ti" href="<?= tenderH(tenderFilterUrl(['cidade' => $city, 'setor' => 'ti', 'situacao' => 'abertas'])) ?>">Ver somente TI aberta</a><a href="<?= tenderH(tenderFilterUrl(['cidade' => $city, 'setor' => 'obras', 'situacao' => 'abertas'])) ?>">Obras</a><a href="<?= tenderH(tenderFilterUrl(['cidade' => $city, 'setor' => 'saude', 'situacao' => 'abertas'])) ?>">Saúde</a><a href="<?= tenderH(tenderFilterUrl(['cidade' => $city, 'setor' => 'seguranca', 'situacao' => 'abertas'])) ?>">Segurança</a></div>
      ...
      <div class="tenders">
        <?php foreach ($procurements as $procurement):
          $isOpen = (int) ($procurement['abertaParaPropostas'] ?? 0) === 1;
          $deadline = strtotime((string) ($procurement['dataFim'] ?? ''));
          $statusLabel = $isOpen ? 'ABERTA PARA PROPOSTAS' : ($deadline !== false && $deadline < time() ? 'PRAZO ENCERRADO' : (string) ($procurement['situacao'] ?? 'NÃO INFORMADA'));
          $itemSector = (string) ($procurement['setor'] ?? 'outros');
          $estimatedValue = (float) ($procurement['valorEstimado'] ?? 0);
        ?>
          <article class="tender<?= $itemSector === 'ti' ? ' ti' : '' ?><?= $isOpen ? '' : ' closed' ?>">
            <div class="tender-top"><div class="badges"><span class="badge"><?= tenderH($procurement['cidade'] ?? '') ?></span><span class="badge sector<?= $itemSector === 'ti' ? ' ti' : '' ?>"><?= tenderH(tenderSectorLabel($itemSector)) ?></span><span class="badge"><?= tenderH(tenderCategoryLabel((string) ($procurement['categoria'] ?? 'outros'))) ?></span></div><span class="status"><?= tenderH($statusLabel) ?></span></div>
            ...
            <dl class="details">
              <div><dt>Setor</dt><dd><?= tenderH(tenderSectorLabel($itemSector)) ?></dd></div><div><dt>Situação no portal</dt><dd><?= tenderH($procurement['situacao'] ?? 'Não informada') ?></dd></div><div><dt>Fonte</dt><dd><?= tenderH(strtoupper((string) ($procurement['fonte'] ?? 'oficial'))) ?></dd></div>
              ...
              <div><dt>Início</dt><dd><?= tenderH(tenderDate($procurement['dataInicio'] ?? null)) ?></dd></div><div><dt>Prazo para propostas</dt><dd><?= tenderH(tenderDate($procurement['dataFim'] ?? null)) ?></dd></div><div><dt>Valor estimado</dt><dd><?= $estimatedValue > 0 ? tenderH(tenderMoney($estimatedValue)) : 'Não informado' ?></dd></div>
            </dl>
            <div class="actions"><a href="<?= tenderH($procurement['urlProcesso'] ?? '#portais') ?>" target="_blank" rel="noopener noreferrer">Conferir processo oficial ↗</a></div>
          </article>
        <?php endforeach; ?>
      ...
      <p class="footnote">Os dados são mantidos no banco do seu site. Uma compra só aparece como aberta quando está no mural em andamento, possui prazo informado e esse prazo ainda não venceu. A classificação por setor é automática e indicativa.</p>
    </section>
